...
def parse_ascii(filename):
  ...
  for line in lines:
    ...
    if not stripped or stripped.startswith("*") or stripped.startswith("."):
      continue
    # A header line: starts with digit string like "0005" or "0006"
    if stripped[:4].isdigit() and len(stripped) >= 8:
      # save previous table
      if current_names is not None and current_data is not None:
        tables.append(current_data)
      # Detect if this is a new variable header: check for 'x' suffix on count
      # Format: NNNNMMMMSSSSxxxxyyyy... then names
      # In HSPICE ASCII, variable names appear on lines starting after the
      # format descriptor
      current_names = None
      current_data  = None
      continue
    # Variable name line (all uppercase words / parenthetical)
    tokens = stripped.split()
    if tokens and all(
      t.replace("(", "").replace(")", "").replace(",", "").replace(".", "")
       .replace("-", "").replace("+", "").replace("e", "").replace("E", "")
       .isalnum() or t.upper() == t
      for t in tokens
    ):
      if current_names is None and any(
        t in ("TIME", "HERTZ", "VOLTS", "AMPS", "FREQ") or t.startswith("V(") or t.startswith("I(")
        for t in tokens
      ):
        current_names = [t.lower().rstrip(",") for t in tokens]
        current_data  = {n: [] for n in current_names}
        continue
    # Data row: all numeric
    if current_names is not None:
      try:
        vals = [float(v) for v in tokens]
        if len(vals) == len(current_names):
          for n, v in zip(current_names, vals):
            current_data[n].append(v)
      except ValueError:
        pass
